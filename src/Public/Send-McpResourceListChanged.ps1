function Send-McpResourceListChanged {
    <#
    .SYNOPSIS
        Sends notifications/resources/list_changed to the clients listening for resource list changes.
    .DESCRIPTION
        The notification goes to every open subscriptions/listen stream whose filter asked for
        resourcesListChanged, tagged with the stream's subscription id. Register-McpResource and Unregister-McpResource
    ...
    .EXAMPLE
        Send-McpResourceListChanged -Server $server
    #>
    ...
    $null = Send-McpServerNotification -Method 'notifications/resources/list_changed' -Context $Context -Server $Server
}
