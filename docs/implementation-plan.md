...
**Dual-Era-Auswahl.** Nachricht mit `_meta`-Pflichtfeldern → zustandslos; `initialize` ohne sie → Legacy-Session (stdio: prozessweit; HTTP: `Mcp-Session-Id` minten, GET-SSE-Stream erlauben, DELETE beenden). Beide Ären parallel auf demselben Endpoint erlaubt. Client (stdio und HTTP gleich): Probe `server/discover` (DiscoverResult → modern; `-32020/-32021/-32022` → modern mit Versionswahl; alles andere → Legacy `initialize`: anderer Fehler, Timeout, HTTP-Fehler ohne JSON-RPC-Body, Ergebnis ohne DiscoverResult-Form); Ära pro Endpoint-URL cachen. Umsetzung und Details: `docs/concepts/dual-era.md`.
...
|---|---|---|
| `New-McpServer` | Server-Objekt anlegen | `-Name -Version [-Title] [-Description] [-Instructions] [-Icons] [-SupportedVersions @('2026-07-28','2025-11-25','2025-06-18')] [-Era Dual\|Modern\|Legacy] [-MaxConcurrency] [-RequestTimeout] [-RequestStateKey <SecureString\|string\|byte[]>] [-RequestStateTtlSeconds] [-DefaultTtlMs] [-PassThru] [-SetDefault]` |
...
- InputRequired-Maschinerie, `requestState`-HMAC (AES-GCM und Principal-Bindung folgen mit M6), Capability-Gating, Elicitation-Validator (Primitiv-Subset, LegacyTitledEnum akzeptieren), Sampling- und Roots-Requests, Client-Retry-Loop mit Callbacks; `subscriptions/listen` Server (Registry, Ack-first, Fan-out, stdio-Cancel, Graceful-Close, dynamische Registrierung) und Client (Hintergrund-Reader, Demux, Reconnect über HTTP).
- **Exit:** alle 14 `input-required-result-*`; die Listen-Checks von `server-stateless` (`server-sse-multiple-streams` prüft nur parallele Request-Streams und war bereits grün); Client `sep-2322-client-request-state`; Pester für Listen auf stdio und HTTP inkl. Interleaving.

### M5 Dual-Era (umgesetzt)
- `McpLegacySession` (Server), Router-Ära-Auswahl, era-bewusste Serialisierung, HTTP-Sessions/GET/DELETE, server-initiierte Requests, Legacy-Elicitation, `ping`, `logging/setLevel`, `resources/subscribe`; Client-Legacy-Lifecycle mit Probe-Erkennung, Session-Header, GET-Stream-Handling, Beantwortung server-initiierter Requests, `-32002`-Akzeptanz, `resultType`-Absenz, SSE-Wiederaufnahme mit `Last-Event-ID`.
- `everything-server.ps1 -Era Dual|Modern|Legacy`; beide Anforderungslisten laufen gegen dieselbe Dual-Instanz.
- **Exit (korrigiert):** `--requirements 2025-11-25` Server 30/30, Client 4/4 außerhalb der Authorization (`initialize`, `tools_call`, `elicitation-sep1034-client-defaults`, `sse-retry`); die 14 `auth/*`-Szenarien dieser Liste gehören zu M6. Das ursprüngliche „Client 18/18“ übersah, dass sie Teil der 18 bewerteten Szenarien sind. `2026-07-28` weiterhin grün auf der Dual-Instanz; Kompatibilitätsmatrix aus `basic/versioning` als Pester-Tabelle (Client Auto/Modern/Legacy × Server modern-only/legacy-only/dual).
- **Abweichungen vom Entwurf:** Die Server-Ära ergibt sich aus `-SupportedVersions` (kein separates `-Era` an `New-McpServer`); Default ist Dual. Ein geschlossener POST-Stream bricht den Handler auch in Legacy-Sessions ab (ohne serverseitige Resumability wäre die Antwort ohnehin verloren). `-32042` wird nicht erzeugt; der blockierende server-initiierte Request ersetzt es.
