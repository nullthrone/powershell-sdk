...
| Folder | Scope | Notes |
|---|---|---|
| `tests/Unit` | manifest, import behaviour, type accelerators, enums against the schema, custom analyzer rules, JSON codec, JSON-RPC model, `_meta` validation, JSON Schema generation and validation (both engines), tool registry and in-process invocation, `server/discover` and the list shapes against the vendored schema, resources (content shaping, not-found errors, caching hints, file and directory resources including traversal and symbolic-link escapes), RFC 6570 template matching, prompts (argument derivation and validation, message shaping), completion (prefix filter, 100-value cap), icons, content annotations, kind-bound cursors, header value encoding and `x-mcp-header` validation, HTTP status mapping, Origin checks, SSE parsing, `requestState` (signature, expiry, binding to method, target and arguments, key derivation), input requests (answers present and missing, `-Defer`, capability gating with `-32021`, answer validation, the elicitation schema subset), the era of a request and the legacy serialization of results and errors, legacy sessions (version negotiation, log level, resource subscriptions), the legacy HTTP status mapping, SSE event ids and retry times | in-process |
| `tests/Integration` | fresh `pwsh` processes: import must be silent on stdout and stderr, also under `-File`; the client against a server in a background runspace (in-memory transport); `examples/echo-server.ps1` over stdio (encoding, 1 MB payloads, progress, timeouts, stderr capture, BOM-free stdout, EOF shutdown), `examples/weather-server.ps1` (all server primitives) and `examples/elicitation-server.ps1` (input requests and a subscription over stdio); resources, prompts, completion, capability gating, log notifications from handler streams and the client cache over the in-memory transport; multi-round-trip requests on the wire (state accumulation, tampering, invalid answers) and through the client callbacks, in memory and over HTTP; `subscriptions/listen` on the wire (acknowledgement, tagging, filter, dynamic registration, cancel, graceful close), through `Register-McpSubscription` (cache invalidation, `-Action`) and as SSE streams (parallel streams, client disconnect); a Streamable HTTP server in a background runspace (client round trips, SSE progress, header mirroring, disconnect → cancel, and raw requests for every status and error code the specification assigns); legacy sessions on the wire (the `initialize` handshake, legacy result shapes, server-initiated elicitation, sampling and roots requests, `logging/setLevel`, unsolicited notifications) in memory and over Streamable HTTP (`Mcp-Session-Id`, GET stream, DELETE, session limits and expiry, request ids of several sessions next to the stateless core); the client against legacy servers (era detection, the compatibility matrix of the versioning page, callbacks for the server's requests, re-initialization after 404, resumption with `Last-Event-ID` against a mock, a legacy session over stdio) | spawns processes, binds loopback ports |
...
The Inspector CLI (`@modelcontextprotocol/inspector`) defaults to the legacy `initialize` handshake, which the
example servers answer in a legacy session (dual era); `"protocolEra": "modern"` makes it use the stateless
lifecycle instead. It parses single-dash arguments such as `-File` as its own options, so describe the server
in a config file:
