...
| **M4 MRTR and subscriptions** | Input-required machinery with signed `requestState`, capability gating, elicitation validator, sampling and roots as input requests, client retry loop; `subscriptions/listen` on server and client, list-changed and resource-updated fan-out, dynamic registration | All 14 `input-required-result-*`; the `subscriptions/listen` checks of `server-stateless` (acknowledgement, subscription id, filter, tools and prompts list changes); client `sep-2322-client-request-state` | Done |
| **M5 Dual era** | Legacy session (`initialize`, `ping`, `logging/setLevel`, `resources/subscribe`, server-initiated requests, HTTP sessions, GET SSE, DELETE), era-aware serialization, client era detection and legacy lifecycle (including SSE resumption with `Last-Event-ID`) | `--requirements 2025-11-25`: server 30/30, client 4/4 outside authorization (`initialize`, `tools_call`, `elicitation-sep1034-client-defaults`, `sse-retry`; the 14 `auth/*` scenarios of the set follow with M6); 2026-07-28 still green on the dual-era instance; compatibility matrix of the versioning page as Pester tests | Done |
| **M6 Authorization** | Client OAuth provider (discovery, PKCE, resource indicators, `iss`, CIMD/DCR/pre-registered clients, scope step-up, token store); server bearer middleware and protected-resource metadata; `requestState` encrypted (AES-GCM) and bound to the authenticated principal | All client `auth/*` scenarios of the 2026-07-28 and 2025-11-25 requirement sets | Planned |
